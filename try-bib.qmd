...
```{python}
# Read the BibTeX files directly -- no intermediate bib->yaml conversion.
# scripts/cvbib.py parses .bib with the standard library only.
import sys
sys.path.insert(0, "scripts")
import cvbib

bibs = cvbib.load_references(["boehm.bib", "boehm-wip.bib"])
data = {"references": bibs}
n_references = len(bibs)
...
for ref_number in range(n_references):
    ...
    if ref_type == "manuscript":
        c_manuscript += 1
        # status comes from an explicit manuscript_type, else the bib `note`
        # field (pandoc maps biblatex note -> note), else defaults to prep
        status = ref.get("manuscript_type", ref.get("note", "in preparation"))
