...
for presentation in sorted_presentations:
    ...
    if presentation.get("type") == "talk":
        markdown_output_talks += foo
    elif presentation.get("type") == "poster":
...
with open("data/service.yaml", "r") as file:
    data = yaml.safe_load(file)
sorted_service = sorted(data.get("service", []), key=lambda x: x.get("date"), reverse=True)
...
for service in sorted_service:
    ...
    if end_date_str is not None:
        ...
        date_formatted = f"{date_formatted} to {end_date_formatted}"
    # handle current values
    if service.get("current") == 'true':
        date_formatted = f"{date_formatted} to present"
